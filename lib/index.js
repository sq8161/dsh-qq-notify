// dsh-qq-notify 宿主插件（安装包版 v1.0.3）
// 自包含实现：腾讯官方 QQ Bot API + 原生扫码绑定（无第三方依赖）
// RPC：通过 ctx.connection.rpc 注册 /qq-notify 通道（loopback 信任域），
// 客户端经同一通道调用 getState / saveState / testSend / bindStart / bindPoll。
// 配置目录：插件自身安装目录（node_modules/dsh-qq-notify/.dsh-qq-notify/），
// 使用 Node 原生 fs 读写，不依赖会话工作区与沙箱权限。
// 等待用户通知：经 session/event 火线监听 ask_user_question / approval/asked /
// plan/mode(active) 三类"等待用户"信号并推送 QQ 提醒。
import { readFileSync, writeFileSync, mkdirSync } from 'node:fs'
const DEFAULT_PRESETS = [
  '【deepseek任务完成】\n项目：{project}\n时间：{time}\n请返回deepseek查看执行结果',
  '', '', '', ''
]
const DEFAULT_INTERRUPT_TEXT = '【deepseek需要你】\n对话正在等待你的选择或确认，请返回查看'
...
const RPC_CHANNEL = '/qq-notify'

// 插件自身安装目录（lib/ 的上一级），配置与凭据存放于此，不依赖工作区
const PKG_DIR = (function () {
  try {
...
// ---------- 插件主体 ----------
export default {
  inject: ['connection', 'subprocess', 'credentials', 'sessionTitle', 'sandboxPolicy', 'agents'],
  async apply(ctx) {
    ...
    const state = {
      ...
      interruptText: DEFAULT_INTERRUPT_TEXT,
      appId: '',
      userOpenid: '',
    }
    ...
    let lastInterruptSession = ''
    let bindRun = null

    // ---------- 路径与持久化（配置固定在插件安装目录；仅当无法解析时兜底会话 cwd） ----------
    async function configBaseDir() {
      if (PKG_DIR) return PKG_DIR
    ...
    async function loadState() {
      ...
      try {
        ...
        if (data && typeof data === 'object') {
          ...
          if (typeof data.interruptText === 'string') state.interruptText = data.interruptText.slice(0, 2000)
    ...
    async function persistState() {
      ...
      try {
        const dir = joinPath(base, CONFIG_DIR)
        mkdirSync(dir, { recursive: true })
        writeFileSync(joinPath(dir, CONFIG_FILE), JSON.stringify({
          enabled: state.enabled,
          active: state.active,
          presets: state.presets,
          interruptText: state.interruptText,
          appId: state.appId,
    ...
    async function dumpDiag(agent, turn, source) {
      try {
        ...
        if (!base) return
        const dir = joinPath(base, CONFIG_DIR)
        const diagPath = joinPath(dir, DIAG_FILE)
    ...
    function buildValues(agent, turn) {
      const session = agent && agent.session
    ...
    function composeText(agent, turn) {
      ...
      return substitute(template, buildValues(agent, turn))
    }

    // 顶层判断：优先 agents.roots()（权威）；其次会话 header 元数据
    // （本部署 session.meta 缺失，但 session.header 恒有 origin/parentSession/delegationDepth）；
    // 兼容旧 session.meta 兜底；最后保守放行。
    ...
    // ---------- 等待用户通知 ----------
    // 通过 session/event 火线识别三类"对话在等人"的信号：
    //   tool/call(name=ask_user_question) —— 模型向用户提问，等待选择/回答
    //   approval/asked —— 有操作等待用户批准
    //   plan/mode(active=true) —— 计划已生成，等待用户确认
    function findAgentBySession(session) {
      if (!agents || !session) return null
      try {
        const list = agents.list ? agents.list() : []
        for (const a of list) {
          if (a && a.session && (a.session === session || a.session.id === session.id)) return a
        }
      } catch (e) { /* 忽略 */ }
      return null
    }

    async function notifyInterrupt(session) {
      if (!state.enabled) return
      const agent = findAgentBySession(session)
      if (!agent || !isTopLevelAgent(agent)) return
      const sessionId = String((session && session.id) || '')
      const now = Date.now()
      if (now - lastInterruptAt < 10000 && sessionId === lastInterruptSession) return
      lastInterruptAt = now
      lastInterruptSession = sessionId
      const text = substitute(state.interruptText || DEFAULT_INTERRUPT_TEXT, buildValues(agent, lastTurn))
      if (!text) return
      try {
        await sendViaQQ(text)
        console.log('[qq-notify] 等待用户通知已发送:', sessionId)
      } catch (e) {
        console.error('[qq-notify] 等待用户通知发送失败:', e && e.message ? e.message : String(e))
      }
    }
    ...
    ctx.on('session/event', (session, event) => {
      try {
        ...
        if (type === 'tool/call') {
          ...
          if (data && data.name === 'ask_user_question') {
            console.log('[qq-notify] 检测到提问等待（ask_user_question）')
            notifyInterrupt(session).catch((e) => {
              console.error('[qq-notify] 提问通知失败:', e && e.message ? e.message : String(e))
            })
          }
        } else if (type === 'approval/asked') {
          console.log('[qq-notify] 检测到审批等待（approval/asked）')
          notifyInterrupt(session).catch((e) => {
            console.error('[qq-notify] 审批通知失败:', e && e.message ? e.message : String(e))
          })
        } else if (type === 'plan/mode') {
          const data = event.data
          if (data && data.active === true) {
            console.log('[qq-notify] 检测到计划确认等待（plan/mode active）')
            notifyInterrupt(session).catch((e) => {
              console.error('[qq-notify] 计划确认通知失败:', e && e.message ? e.message : String(e))
            })
          }
        }
      } catch (e) {
        console.error('[qq-notify] session/event 处理失败:', e && e.message ? e.message : String(e))
      }
    })
    ...
    // ---------- 客户端 RPC 通道（loopback 信任域；端点 = 上面的方法名） ----------
    const connection = ctx.get('connection')
    if (connection && connection.rpc && connection.rpc.handle) {
      const dispose = connection.rpc.handle(RPC_CHANNEL, async (endpoint, payload) => {
        try {
          ...
          return { ok: false, error: { code: 'internal', message: e && e.message ? String(e.message) : String(e), details: {} } }
        }
      }, { authority: 'loopback' })
      ctx.on('dispose', () => {
        try { dispose() } catch (e) { /* 忽略 */ }
      })
      console.log('[qq-notify] RPC 通道已注册:', RPC_CHANNEL)
    } else {
      console.error('[qq-notify] connection 服务不可用，客户端设置页将无法工作')
    }
    ...
    console.log('[qq-notify] 插件已启动（安装包版 v1.0.3）：配置目录 =', await configBaseDir() || '(无)')
  },
}
