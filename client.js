// DSH QQ 通知插件 v17 - Client 半部（底部日志改弹窗；手册变量按 A-Z 排序）
// 自包含实现：SVG 二维码由内嵌纯 JS QR 编码器生成（无 iframe、无第三方二维码 API）
// 注意：本文件为开源镜像，实际运行时由动态插件系统加载（Client 代码运行在浏览器，
// 无 import/require/JSX，React 组件须用 React.createElement 构建）。
...
`)
    ...
    function QQNotifySettings() {
      ...
      React.useEffect(() => {
        if (!toast) return
        const stop = ctx.setTimeout(() => { setToast(null) }, toast.isError ? 5000 : 3200)
        return stop
      }, [toast ? toast.id : null])
      ...
      const save = () => {
        ...
        host.call('saveState', args).then((r) => {
          setBusy(false)
          if (r && r.ok) {
            setAppSecret('')
